...
import { useVendorSalesReportQuery } from './vendor-report-api-slice';
import { VendorReportFilter } from './vendor-report-filter';

export function VendorSalesReportPage() {
	...
	const { data, isLoading, isError, isFetching } = useVendorSalesReportQuery({
		page: page,
					...
					<>
						<div className="border rounded-lg relative">
							...
							<Table>
								...
								<TableBody>
									{data.sales_report.data.length === 0 ? (
										<TableRow>
											<TableCell
												colSpan={7}
										...
										data.sales_report.data?.map((item, i) => (
											<TableRow key={item.id}>
												<TableCell className="py-2 pl-4">
													{tableSrCount(data.sales_report.current_page, i)}
												</TableCell>
						...
						<Pagination1 pagination={data?.sales_report} setPage={setPage} />
					</>
				)}
			</Container1>
		</>
