...
export function ProductSection5() {
	...
	const form = useForm<ZodType>({
		...
		defaultValues: {
			best_section_title: data?.data?.extra_section_tittle_5 || '',
			best_category_id: data?.data?.recomended_category_id_2 || '',
			best_sub_category_id: data?.data?.recomended_category_id_2 || '',
		},
	});
	...
	useEffect(() => {
		if (data?.data) {
			form.reset({
				best_section_title: data?.data?.extra_section_tittle_5 || '',
				best_category_id: data?.data?.recomended_category_id_2 || '',
				best_sub_category_id: data?.data?.recomended_sub_category_id_2 || '',
			});
			...
			onOk: async () => {
				try {
					const response = await store({
						extra_section_tittle_5: data.best_section_title,
						recomended_category_id_2: data.best_category_id,
						recomended_sub_category_id_2: data.best_sub_category_id,
					}).unwrap();
