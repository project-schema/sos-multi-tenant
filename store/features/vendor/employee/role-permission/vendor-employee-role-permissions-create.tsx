...
import { buildRolePermissionPayload, checkedTotal } from './role-permission-utility';
import { useVendorEmployeeRoleCreateMutation } from './role-permissions-api-slice';
...
export function VendorEmployeeCreateRole() {
	const [open, setOpen] = useState(false);
	const [state, dispatch] = useReducer(reducer, initialState);
	const [store, { isLoading }] = useVendorEmployeeRoleCreateMutation();
	...
	const onSubmit = (data: ZodType) => {
		...
		alertConfirm({
			onOk: async () => {
				dispatch({
					type: 'API_VALIDATION',
					payload: { name: null, permission: null },
				});

				const formData = buildRolePermissionPayload(data.name, checkedAll);

				try {
					const res = await store(formData).unwrap();
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger asChild>
				<Button>Create Employee Role</Button>
			</DialogTrigger>

			<DialogContent className="w-full sm:max-w-5xl overflow-y-auto max-h-[90vh]">
				<DialogHeader>
					<DialogTitle>Create Employee Role</DialogTitle>
					<DialogDescription>Create a role for employees.</DialogDescription>
				</DialogHeader>
