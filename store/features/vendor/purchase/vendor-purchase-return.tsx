...
export function VendorPurchaseReturn({ data }: { data: iVendorPurchaseShow }) {
	...
	const form = useForm<ReturnFormData>({
		...
		defaultValues: {
			return_items: purchase_show.purchase_details?.map((item) => ({
			...
			onOk: async () => {
				try {
					...
					const response = await submitReturn({
						id: purchase_show.id,
						return_qty: returnData?.map((item) => item?.return_qty),
						remark: returnData?.map((item) => item?.remark),
					}).unwrap();
			...
			<Card>
				<CardHeader>
					<CardTitle className="flex items-center justify-between">
						<span>Invoice #{purchase_show.chalan_no}</span>
						<Badge variant="outline">{purchase_show.purchase_date}</Badge>
					</CardTitle>
				</CardHeader>
				<CardContent>
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div>
							...
							<div className="space-y-1 text-sm">
								<p className="font-medium">
									{purchase_show.supplier.supplier_name}
								</p>
								<p>{purchase_show.supplier.phone}</p>
								<p>{purchase_show.supplier.email}</p>
								<p>{purchase_show.supplier.address}</p>
							</div>
						</div>
						...
						<div>
							...
							<div className="space-y-1 text-sm">
								<p className="font-medium">{logo?.shop_name}</p>
								<p>{logo?.phone}</p>
								<p>{logo?.email}</p>
								<p>{logo?.address}</p>
							</div>
