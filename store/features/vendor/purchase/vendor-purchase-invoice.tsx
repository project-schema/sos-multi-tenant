...
export function VendorPurchaseInvoice({ data }: { data: iVendorPurchaseShow }) {
	const { logo, purchase_show } = data;

	// Format purchase date
	const purchaseDate = purchase_show.purchase_date;

	const invoiceTime = purchase_show.created_at
		? timeFormat(purchase_show.created_at)
		: '';

	// Calculate totals
	const totalQty = purchase_show.purchase_details.reduce(
		(sum, item) => sum + item?.qty,
		0
	);
	const totalPrice = parseFloat(purchase_show.total_price) || 0;
	const discount = parseFloat(purchase_show.purchase_discount) || 0;
	const paidAmount = parseFloat(purchase_show.paid_amount) || 0;
	const dueAmount = parseFloat(purchase_show.due_amount) || 0;
		...
		<div className="max-w-4xl mx-auto border border-gray-300 rounded-lg p-4 bg-white print:p-0">
				...
				<div className="border border-gray-300 rounded p-4">
					...
					<div className="space-y-1">
						<p className="font-medium text-gray-900">
							{purchase_show.supplier.supplier_name || 'name'}
						</p>
						<p className="text-gray-700">
							{purchase_show.supplier.address || 'address'}
						</p>
						<p className="text-gray-700">
							{purchase_show.supplier.phone || 'phone'}
						</p>
						<p className="text-gray-700">
							{purchase_show.supplier.email || 'email'}
						</p>
					</div>
				</div>
			...
			<div className="mb-6">
				<table className="w-full border-collapse border border-gray-300">
					...
					<tbody>
						{purchase_show.purchase_details.map((item, index) => (
							<tr
								key={item?.id}
			...
			<div className="border-t-2 border-gray-300 pt-6">
				<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
					...
					<div className="text-center">
						...
						<p className="font-semibold text-lg">{purchase_show.chalan_no}</p>
					</div>
				</div>
