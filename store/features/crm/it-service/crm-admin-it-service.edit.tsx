...
import { useAdminUpdateCrmITServiceMutation } from './crm-admin-it-service.api.slice';
import { iCrmITService } from './crm-admin-it-service.type';
	...
	const onSubmit = async (data: ZodType) => {
		try {
			...
			if (response.status === 200) {
				toast.success(response.message || 'ITService updated successfully');
				setOpen(false);
			} else {
					...
					<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
						...
						<DialogFooter>
							<Button type="submit" disabled={isLoading}>
								...
								{isLoading ? 'Updating...' : 'Update ITService'}
							</Button>
						</DialogFooter>
					</form>
				</Form>
