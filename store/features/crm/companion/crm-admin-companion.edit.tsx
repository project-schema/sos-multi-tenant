...
import { useAdminUpdateCrmCompanionMutation } from './crm-admin-companion.api.slice';
import { iCrmCompanion } from './crm-admin-companion.type';
...
export function CrmCompanionEdit({
	editData,
}: {
	editData: iCrmCompanion;
}) {
	...
	const onSubmit = async (data: ZodType) => {
		try {
			...
			if (response.status === 200) {
				toast.success(
					response.message || 'Companion updated successfully'
				);
				setOpen(false);
			} else {
