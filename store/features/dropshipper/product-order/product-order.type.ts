...
export type iDropShipperProductOrder = {
	id: number;
	vendor_id: number;
	product_id: number;
...
export type iDropShipperOrderStatusType =
	| 'hold'
	| 'pending'
	| 'received'
	| 'processing'
...
export type iDropShipperProductOrderResponse = {
	status: number;
	message: iPagination<iDropShipperProductOrder>;
};

export type iDropShipperProductOrderStatistics = {
	status: number;
	data: string;
	message: {
		totalorder: number;
